...
`ES6`模块规范是使用`import`语句导入模块，`export`语句导出模块，输出的是对值的引用
...
:::
### `ES6` 模块加载原理
>`ES6` 模块与`CommonJS`有本质上的区别。`ES6` 模块是动态引用，遇到模块加载命令`import`时不会立即执行模块，只是生成一个指向被加载模块的引用。
```js
//odd.js
...
`ES6`模块的运行机制和`CommonJS`不一样，遇到模块加载命令`import`时不去执行这个模块，只会生成一个动态的只读引用，等到需要用到这个值时，再到模块中取值，也就是说模块中的值变了，那输入的值也会发生变化
