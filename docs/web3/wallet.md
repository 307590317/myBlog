...
```jsx
// 生成随机熵：16 字节 (128 bit) → 12 个单词（最常用，MetaMask 默认）
const entropy = ethers.randomBytes(16);
// 将随机熵转化为 12 位助记词 (Mnemonic)
...
### 种子 (Seed)
:::tip
助记词 → 通过 PBKDF2 算法 → 得到一个 512 位的 种子（512位的随机数）。是后续生成私钥、公钥、地址的根基。
PBKDF2进行2048 次迭代，这么多次迭代是为了增加暴力破解的成本
:::

### 私钥 (Private Key 32字节随机数)
:::tip
通过 HMAC-SHA512 算法生成主私钥，一个非常大的随机数（通常是 256 位的整数， 即256 位 = 32 字节 = 64 个十六进制字符）。 
随后根据派生路径，推导出特定层级的子私钥。
派生路径：
...
- coin_type：区分不同的区块链，0'代表 BTC，60'代表 ETH
- account：用于区分用户的不同账户，从 0'开始，递增
...
### 公钥
使用 ECDSA（椭圆曲线加密） 算法，通过 K = 私钥 * G 计算出公钥点。根据公钥点坐标+不同的规则得出压缩/未压缩公钥。
...
得到的公钥可以是：
  -	未压缩公钥：65字节，0x04 + X坐标(32字节) + Y坐标(32字节)
  -	压缩公钥：33字节，0x02/0x03 + X坐标(32字节)
