...
React 使用 Virtual DOM 的核心目的，不是“绝对更快”，而是：
 1.	提供声明式 UI 编程模型
开发者只需要描述“状态对应什么 UI”，不用自己精细操作真实 DOM。
 2.	让跨平台渲染成为可能
React 的核心 reconciler 不直接依赖浏览器 DOM，因此可以扩展到 React Native、自定义 renderer。
 3.	通过 Diff 降低不必要的 DOM 操作
DOM 操作成本通常高于 JS 对象操作，React 先在内存中比较，再最小化更新真实 DOM。
...
React 的真正价值不是 “Virtual DOM 比真实 DOM 快”，而是它把 UI 更新抽象成可预测、可组合、可调度的过程。
...
核心假设有两个：
 1.	不同类型的元素会产生不同的子树
例如 `<div>` 变成 `<span>`，React 会直接销毁旧树并重建新树。
 2.	开发者可以通过 key 告诉 React 哪些子元素是稳定的
这样 React 可以高效比较同层级列表。
...
用户输入、动画这类高优先级任务可以先处理。
 4.	支持并发特性
比如 Concurrent Rendering、Suspense、Transition 等。
...
Fiber 不是某个 API，而是 React 底层协调器的重写。它最大的价值是让 React 从“同步递归渲染框架”升级成“可调度渲染框架”。
...
这也是为什么在 render 期间不能做请求、订阅、DOM 操作等副作用。
...
React 内部并不是通过变量名识别 Hook，而是通过 调用顺序 来关联每个 Hook 对应的状态槽位。
...
Hooks 的本质不是魔法，而是基于链表/数组式的内部存储结构，通过调用顺序定位状态。如果改变调用规则，会破坏Hooks的调用顺序。
...
React 18 的并发（Concurrent Rendering）不是多线程并行，而是：

React 可以同时准备多个版本的 UI，并按优先级调度更新，让渲染过程更平滑、更可中断。

它的重点不是“同时跑多个线程”，而是“更聪明地安排主线程上的渲染工作”。
...
并发模式下，某次 render 结果不一定会 commit；React 可能中途打断、重算，因此 render 阶段更要求纯函数化。


## React 18 的自动批处理（Automatic Batching）是什么？

批处理指 React 把多个 state 更新合并成一次渲染，以减少不必要的 re-render。
...
闭包陷阱常见于 Hooks 中：函数捕获了旧的 state/props，后续异步执行时拿到的不是最新值。
...
```
如果点击后继续更新 count，1 秒后打印的可能仍是点击当时的旧值。
...
React.memo 用于缓存函数组件的渲染结果。当 props 没有变化时，React 可以跳过该组件重新渲染。
```jsx
...
3）组件内部 useContext 更新

即使 props 不变，如果消费的 context 变了，组件仍会 re-render。
...
memo 不是阻止组件“执行”，而是告诉 React 在 props 未变化时可以跳过这次渲染工作。
...
当 Provider 的 value 引用变化时，所有消费该 context 的组件都可能重新渲染。

2）不适合高频更新的大型状态

比如表单输入、复杂全局状态，如果全部塞进一个 Context，性能会比较差。

3）可维护性问题

Context 过多或 value 结构过大时，依赖关系不清晰，调试困难。
...
如果 reducer 有副作用，重放 action 时结果可能不一致。
...
我一般先用 Profiler 看具体是哪个组件在高频重复渲染，再判断是 props 引用不稳定、context 扩散、状态设计不合理，还是列表渲染量过大。然后再针对性使用 memo、useMemo、useCallback、虚拟列表、代码分割等手段，而不是一上来全局加优化。
...
HOC、render props、custom hooks 都能做逻辑复用，但 Hooks 在组合性和嵌套使用体验上通常更自然。
...
2）Render Props：把逻辑结果通过函数子组件传出去，JSX 嵌套较深，容易出现冲突
...
服务器先生成完整 HTML 返回给浏览器，首屏更快、SEO 更好，然后客户端再 hydrate 接管交互。
...
SSR 返回的是可展示 HTML，但要等客户端 JS 执行后，React 才会把静态页面“激活”为可交互应用，这个过程就是 hydration。
...
也就是说，在同样的 state 下，组件应该稳定地产生同样的 UI 输出。
...
如果能提到“按业务边界组织代码，而不是纯技术类型组织”，会更像有实战经验的人。


面试中的高分答题技巧

遇到 React 进阶题，建议按这个结构回答：
...
我会先定位瓶颈，再决定是做引用稳定、组件拆分、虚拟列表还是代码分割，避免过度优化。
