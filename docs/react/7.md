...
可以补充：
 ...
 - key 的存在，是为了让React 在同级节点比较时，判断是否可以复用以及如何服用。
...
**总结：**  
Hooks 的本质是基于链表/数组式的内部存储结构，通过调用顺序定位状态。如果改变调用规则，会破坏Hooks的调用顺序。
...
React 可以同时准备多个版本的 UI，并按优先级调度更新，让渲染过程更平滑、可中断。
...
## useRef 有哪些典型用途？
useRef 原理是：初次渲染时挂载一个 memoizedState 对象，之后渲染直接返回这个对象，不进行任何比对或更新逻辑。
...
表示：界面是状态的映射结果。在同样的 state 下，组件应该产生同样的 UI 输出。
