...
但是由于调用`pushState`和`replaceState`不会触发事件（`popstate`事件只有在用户手动点击前进、后退按钮或者在js中调用`go、back、forward`方法时才会触发），所以有两种解决方案：
- 1、点击前进、后退按钮或者在`js`中调用`go、back、forward`方法：监听`popstate`事件，在事件回调中根据当前的url，渲染对应的页面即可。
- 2、当需要跳转路径时：先根据拿到的路径渲染对应的页面，然后在页面渲染完成之后通过`pushState`或`replaceState`方法来更新浏览器的url。
...
let router = new VueRouter({//初始化路由：传入路由表
  mode:'history',
  routes //路由中提供的路由表属性名就是routes所以可以简写如下
});
