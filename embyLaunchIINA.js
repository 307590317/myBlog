...
(function () {
      ...
      if (mediaSource.Path.startsWith("http") && localStorage.getItem(lsKeys.strmDirect) === "1") {
          ...
          if (isEmby) {
              if (mediaSource.IsInfiniteStream) {
                  ...
                  streamUrl += useRealFileName ? `stream/${fileName}` : `file`;
              }
          } else {
              streamUrl += `Download`;
  ...
  function updateToken(){
    if (location.port === "19798") { // CloudDrive2
        const token = localStorage.getItem("token");
        if (token) {
            GM_setValue("clouddrive_token", token); // 储存到油猴的隔离存储
        }
    }else if (location.port === "8096") {
        const cdToken = GM_getValue("clouddrive_token");
        localStorage.setItem('cd2t', cdToken)
    }
  }
